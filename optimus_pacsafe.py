...
REVIEW_SHEET_NAME = "팩세이프네이버리뷰" # 시트 이름 확인 필요
...
def process_logic(token):
    ...
    # 4. 월별 시트 업데이트 (원가/정산 로직은 간소화하여 필수 데이터만 전송)
    by_month = {}
    for item in orders:
    ...
    for m, items in by_month.items():
        ...
        # 기존 데이터 유지하며 업데이트 (간단한 Append 방식 사용)
